...
void main() {
  ...
  test('25 stake commits once, wins net 75, and survives reload', () async {
    ...
    expect(service.loadCoins().balance, 475);
    final coins = await service.settleGame(gameId: 'win', stake: 25, humanWon: true);
    ...
    expect(PreferencesService(preferences).loadCoins().toJson(), coins.toJson());
  });

  test('25 stake loss loses only the committed stake', () async {
    await service.commitStake('loss', 25);
    final coins = await service.settleGame(gameId: 'loss', stake: 25, humanWon: false);
  ...
  test('Free Play wins 10 and loses zero, including at zero balance', () async {
    final preferences = await SharedPreferences.getInstance();
    await preferences.setString(PreferencesService.coinStateKey,
  ...
  test('concurrent and older duplicate IDs cannot deduct or settle twice', () async {
    await Future.wait(List.generate(3, (_) => service.commitStake('one', 25)));
    expect(service.loadCoins().balance, 475);
    await Future.wait(List.generate(3, (_) => service.settleGame(
      gameId: 'one', stake: 25, humanWon: true)));
  ...
  test('replacing a game has no refund; normal reset keeps coin history', () async {
  ...
  testWidgets('zero balance shows on Home and only Free is available', (tester) async {
  ...
  testWidgets('Continue restores stake without selector or another deduction; result shows net', (tester) async {
